...
function(ngm_add_resource_workers)
    ...
    foreach(profile IN ITEMS 2026_3 2026_2)
        ...
        foreach(index RANGE 0 4)
            ...
            string(JSON files SET "${files}" "${name}" "\"${expected}\"")
        endforeach()
        string(JSON expected_profiles SET "${expected_profiles}" "${profile}"
            "{\"profile\":\"${identity}\",\"files\":${files}}")
    ...
    endforeach()
    file(CONFIGURE OUTPUT "${CMAKE_CURRENT_BINARY_DIR}/generated/ResourceProfiles.hpp" CONTENT
        "#pragma once\nnamespace ngm { inline constexpr char resource_profiles_json[] = R\"ngm_profiles(${expected_profiles})ngm_profiles\"; }\n" @ONLY)
    target_include_directories(ngm_inspection PRIVATE "${CMAKE_CURRENT_BINARY_DIR}/generated")
endfunction()
